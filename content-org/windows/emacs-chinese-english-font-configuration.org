:PROPERTIES:
:ID:       a3c1f7d2-8b9e-4f12-a6d5-3e2b8c4d9f01
:END:
#+TITLE: Emacs 中英文字体配置指南
#+HUGO_TAGS: Emacs 字体 中英文 org-mode
...
4. =关闭多余插件的字体切换逻辑= —— 许多 major mode（如 =lsp-mode= , =company= , =tree-sitter= ）会在不同上下文下切换字体，触发额外的 fontset 查询。可以通过 =(setq inhibit-compacting-font-caches t)= 减少缓存抖动。
...
[[https://github.com/be5invis/Sarasa-Gothic][Sarasa Gothic]] 由 Iosevka 的作者 Belleve Invis 开发，将 Iosevka 与思源黑体（Source Han Sans）合并。它保留了 Iosevka 的编程连字和等宽特性，同时完整支持 CJK 字符集。Sarasa Mono SC 是等宽无衬线版本， =Nerd Font= 变体额外包含 Powerline 符号和文件类型图标。
...
[[https://font.subf.dev/zh-cn/][Maple Mono]] 是一款专为编程设计的字体，其 CN 变体内置中文支持，同样保证中英文 2:1 等宽。还提供 Nerd Font 变体（ =Maple Mono NF CN= ），包含 Powerline 符号。
...
* 方案二：中英文字体分别设置 + face-font-rescale-alist
...
=注意= ：修改 =face-font-rescale-alist= 后 =必须重启 Emacs= 才能生效（这是一个 [[https://debbugs.gnu.org/db/17/1785.html][已知的 bug]] ）。
...
** 配置 default / fixed-pitch / variable-pitch
...
* 方案三：使用 fontset 为特定 Face 单独设置字体
...
| =org-verbatim=         | 等宽文本（ ~=verbatim=~ ） | 保持等宽显示     |
...
** 为什么修改 face-font-rescale-alist 后不生效？

=face-font-rescale-alist= 修改后 =必须重启 Emacs= 。这是一个 [[https://debbugs.gnu.org/db/17/1785.html][已知的 bug]] （#1785），从 2010 年报告至今。不要试图用 =eval-buffer= 或重新求值来让它生效，直接重启。
...
** set-frame-font 和 set-face-attribute 有什么区别？
...
- [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Fonts.html][Emacs Manual - Fonts]]
- [[https://www.gnu.org/software/emacs/manual/html_node/modus-themes/DIY-Font-configurations-for-Org-and-others.html][Modus Themes - Font configurations for Org]]
- [[https://idiocy.org/emacs-fonts-and-fontsets.html][Emacs, fonts and fontsets - Alan Third]]
- [[https://github.com/be5invis/Sarasa-Gothic][Sarasa Gothic - Iosevka + 思源黑体]]
- [[https://github.com/be5invis/Iosevka][Iosevka - 可变编程字体]]
- [[https://font.subf.dev/zh-cn/][Maple Mono - 编程字体]]
- [[https://github.com/lxgw/LxgwWenKai][LXGW WenKai - 落霞孤鹜文楷]]
- [[https://zzamboni.org/post/beautifying-org-mode-in-emacs/][Beautifying Org Mode in Emacs]]
- [[https://emacsredux.com/blog/2023/03/16/setting-the-default-font-for-emacs/][Setting the Default Font for Emacs]]
- [[https://jihuan-tian.github.io/computer/2026/04/29/font-configuration-in-emacs.html][Font configuration in Emacs - Jihuan Tian]]
